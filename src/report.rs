...
pub fn generate_html_report(path: &str, results: &[ScanResult], total_elapsed: Duration) -> Result<()> {
    ...
    for r in results {
        ...
        for p in r.open_ports() {
            records.push(ExportRecord::from_scan_result(&ip_str, r.hostname.as_ref(), p));
        }
    }
    ...
    let html_string = template.render().map_err(|e| {
        // We'll wrap Askama errors in io errors for simplicity, or any valid fallback
        RustScanError::Io(std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))
    })?;
